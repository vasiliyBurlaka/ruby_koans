...
class Proxy
  ...
  def method_missing (method_name, *args, &block)
    if @object.respond_to? method_name 
      @messages[method_name.to_sym] = (@messages[method_name.to_sym] == nil) ? 1 : @messages[method_name.to_sym] + 1 
  ...
  def called?(arg)
    @messages.keys.include? arg 
  end

  def number_of_times_called(method)
    @messages[method] == nil ? 0 : @messages[method]
  end
end
